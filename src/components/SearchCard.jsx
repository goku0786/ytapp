    ...
    <div>
      <Link to={`/video/${video?.videoId}`}>
        <div className="flex flex-col sm:flex-row px-5">
          <div className="relative flex h-48  w-full md:w-[300px] overflow-hidden ">
            <img
              className="h-full w-full rounded-lg object-cover"
              src={video?.thumbnails[0]?.url}
            />
            <div>{video?.lengthSeconds && <Time time={video?.lengthSeconds} />}</div>
          </div>
          <div className="flex flex-col ml-4 md:ml-6 mt-4 md:mt-0 overflow-hidden">
            <span className=" md:text-xl font-semibold line-clamp-2 ">
              {video?.title}
            </span>
